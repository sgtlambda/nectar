        ...
        _.each(paths, path => {
            queue = queue.then(() => pify(fs.stat)(path)
                .then(stat => {
                    ...
                    let entry;
                    let addEntry   = ()=> entry = pack.entry(_.assign(stat, {name: path}));
                    // Weird safety net type thing
                    try {addEntry();} catch (e) {setTimeout(addEntry, 50);}
                    return new Promise((resolve) => {
                        readStream.on('end', () => setTimeout(resolve, 0));
