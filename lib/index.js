...
const packFile = function (file, pack) {
    return pify(fs.stat)(file)
        .then(stat => !stat.isFile() ? Promise.resolve(false) : new Promise((resolve, reject) => {
...
const packAll = function (paths, pack) {
    let entries = [];
    return _.reduce(paths, (queue, path) => queue.then(() => packFile(path, pack).then(packed => {
            if (packed) entries.push(path);
        })), Promise.resolve())
        .then(() => entries);
};
